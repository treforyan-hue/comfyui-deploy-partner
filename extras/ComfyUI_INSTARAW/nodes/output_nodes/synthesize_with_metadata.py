...
class INSTARAW_SynthesizeAuthenticMetadata:
    ...
    def synthesize_and_save(
        ...
        # Combine subfolder with filename prefix (ComfyUI supports "subfolder/prefix" format)
        if output_subfolder.strip():
            combined_prefix = os.path.join(output_subfolder.strip(), "IMG")
        else:
            combined_prefix = "IMG"
