...
class INSTARAW_SaveWithAuthenticMetadata:
    ...
    def save_image(self, image: torch.Tensor, output_subfolder: str, filename_prefix: str, profile_path=None):
        ...
        # Combine subfolder with filename_prefix (ComfyUI supports "subfolder/prefix" format)
        if output_subfolder.strip():
            combined_prefix = os.path.join(output_subfolder.strip(), filename_prefix)
        else:
            combined_prefix = filename_prefix

        full_output_folder, filename, counter, subfolder, _ = folder_paths.get_save_image_path(combined_prefix, output_dir, img_pil.width, img_pil.height)
