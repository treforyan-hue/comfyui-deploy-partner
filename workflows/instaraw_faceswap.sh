#!/usr/bin/env bash
# INSTARAW Faceswap — outfit + hair + character swap via SAM HQ + GroundingDino
# GPU: 12GB+ | Models: ~4GB | Time: ~3 min
# Uses INSTARAW API nodes (Gemini/Ideogram/Grok) — user provides their own API keys

models_instaraw_faceswap() {
    section "Models: INSTARAW Faceswap (segmentation-based)"

    # SAM HQ ViT-H + GroundingDino (auto-downloaded by storyicon/comfyui_segment_anything)
    # but we pre-download to avoid first-run delay
